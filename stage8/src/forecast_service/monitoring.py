...
KS_GATE = 0.2             # статистика Колмогорова-Смирнова выше - заметный сдвиг распределения
...
def gate(accuracy: dict | None, drift: list | None, breakdowns: dict | None = None,
    ...
    if drift:
        wide = [f["feature"] for f in drift
                if (f.get("psi") is not None and f["psi"] > PSI_GATE)
                or (f.get("ks") is not None and f["ks"] > KS_GATE)]
        if wide:
            warnings.append(f"дрейф признаков: {', '.join(wide)} (PSI выше {PSI_GATE} или KS выше {KS_GATE})")
    if breakdowns:
